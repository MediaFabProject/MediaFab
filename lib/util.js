...
export class SettingsManager {
    ...
    static async saveParamountPlusSettings(config) {
        await AsyncLocalStorage.setStorage({ paramountplus_settings: config });
    }

    static async getParamountPlusSettings() {
        const result = await AsyncLocalStorage.getStorage(["paramountplus_settings"]);
        return result.paramountplus_settings || {};
    }

    static async saveBBCIPlayerSettings(config) {
        await AsyncLocalStorage.setStorage({ bbc_iplayer_settings: config });
    }

    static async getBBCIPlayerSettings() {
        const result = await AsyncLocalStorage.getStorage(["bbc_iplayer_settings"]);
        return result.bbc_iplayer_settings || {};
    }
    ...
    static async saveMediaFabCompanionConfig(config) {
        await AsyncLocalStorage.setStorage({ mediafab_companion_config: config });
    }

    static async getMediaFabCompanionConfig() {
        const result = await AsyncLocalStorage.getStorage(["mediafab_companion_config"]);
        const config = {
            enabled: false,
            projectFolder: "",
            destination: "",
            ...(result.mediafab_companion_config || {}),
        };
        const oldFolderName = "/widevineproxy2-multimode-companion";
        if (config.projectFolder.endsWith(oldFolderName)) {
            config.projectFolder = `${config.projectFolder.slice(0, -oldFolderName.length)}/MediaFab Queue Mode Companion`;
            await this.saveMediaFabCompanionConfig(config);
        }
        return config;
    }
...
export class IconManager {
    static setDefaultIcon() {
        chrome.action.setIcon({
            path: {
                16: "images/mediafab-icon-green-lavender-light-16.png",
                32: "images/mediafab-icon-green-lavender-light-32.png",
                48: "images/mediafab-icon-green-lavender-light-48.png",
                128: "images/mediafab-icon-green-lavender-light-128.png"
            }
        });
    }
