export class RemoteCdm {
    ...
    async set_service_certificate(session_id, certificate_b64) {
        const certificate_request = await fetch(
            `${this.host}/${this.device_name}/set_service_certificate`,
            {
                method: "POST",
                headers: {
        ...
        console.log("[WidevineProxy2]", "REMOTE_CDM", "SET_SERVICE_CERTIFICATE", certificate_request.status);
        if (!certificate_request.ok) {
            throw new Error("Remote CDM rejected the service certificate");
        }
    }
