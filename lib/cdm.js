...
export class Session {
    constructor(contentDecryptionModule, pssh) {
        ...
        if (typeof pssh === "string") {
            pssh = Util.b64.decode(pssh)
        }
        this._pssh = pssh
    }

    getLicenseChallenge(messageBytes, encryptClientId = false) {
        let signedMessage;
        try {
            signedMessage = SignedMessage.decode(new Uint8Array(messageBytes));
        } catch {
            return null;
        }
        ...
        const widevineData = licenseRequest.contentId?.widevinePsshData;
        if (!widevineData?.psshData?.[0]) {
            throw new Error("the license request does not contain Widevine PSSH data");
        }

        delete licenseRequest.encryptedClientId;
        delete licenseRequest.clientId;
        if (encryptClientId && this._serviceCertificate) {
        ...
        const requestId = this._deviceType === 2
            ? this._generateAndroidIdentifier()
            : this._generateGenericIdentifier();
        widevineData.requestId = requestId;
        ...
        this._pssh = widevineData.psshData[0];
        ...
        md.update(Util.utf8.decode(this._rawLicenseRequest), "raw");
        signedMessage.msg = this._rawLicenseRequest;
    ...
    createLicenseRequest(licenseType = LicenseType.STREAMING, android = false) {
        if (Util.sequenceEquals(this._pssh.subarray(4, 8), PSSH_MAGIC)) {
            if (Util.sequenceEquals(this._pssh.subarray(12, 28), WIDEVINE_SYSTEM_ID)) {
                this._pssh = this._pssh.subarray(32)
            } else {
                throw new Error(`Invalid System ID in PSSH Box: ${Util.bytesToHex(this._pssh.subarray(12, 28))}`)
            }
        }
        ...
        const signature = Util.utf8.encode(this._devicePrivateKey.sign(md, pss))
    ...
    async parseLicense(rawLicense) {
        ...
        const sessionKey = this._devicePrivateKey.decrypt(
            Util.utf8.decode(signedLicense.sessionKey),
            "RSA-OAEP",
            {
                md: forge.md.sha1.create()
            }
        ...
        if (signedLicense.oemcryptoCoreMessage) {
            hmac.update(Util.utf8.decode(signedLicense.oemcryptoCoreMessage))
        }
        hmac.update(Util.utf8.decode(signedLicense.msg))
        const calculatedSignature = Util.utf8.encode(hmac.digest().data)

        if (!Util.sequenceEquals(calculatedSignature, signedLicense.signature)) {
        ...
        const keyContainers = license.key.map(keyContainer => {
            if (keyContainer.type && keyContainer.type === 2 && keyContainer.key && keyContainer.iv) {
                ...
                return {
                    kid: keyContainer.id.length !== 0 ? Util.bytesToHex(keyContainer.id) : "00000000000000000000000000000000",
                    k: Util.bytesToHex(Util.utf8.encode(decipher.output.data))
                }
            }
        })
    ...
    _generateAndroidIdentifier() {
        ...
        return Util.utf8.encode(`${Util.bytesToHex(randomBytes).toUpperCase()}000000000100000000000000`)
    }
    ...
    static psshDataToPsshBoxB64(pssh_data) {
        ...
        const pssh = new Uint8Array([
            ...Util.u32toBytes(totalLength),
            ...PSSH_MAGIC,
            ...new Uint8Array(4),
            ...WIDEVINE_SYSTEM_ID,
            ...Util.u32toBytes(dataLength),
            ...pssh_data
        ]);
        return Util.b64.encode(pssh);
    }
