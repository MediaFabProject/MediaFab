...
test('Queue refreshes the selected getter together with its matching shared configuration', () => {
    ...
    const [job] = buildBatchJobs(
        createManualLinkCatalog(['https://www.crunchyroll.com/watch/GE00362087ENUS/example']),
    ...
    assert.match(command, /Media Metadata and Extras Getter\/Launchers\/media_metadata_and_extras_getter\.py/);
...
test('Queue Mode preserves an already-selected series folder for the normal MME workflow', () => {
...
test('Queue Mode sends each capture through the shared normal command builder', () => {
    const watchUrl = 'https://www.crunchyroll.com/watch/GE00362091ENUS/episode-three';
    const catalog = createManualLinkCatalog([watchUrl]);
    ...
    assert.match(command, /--save-name "crunchyroll-GE00362091ENUS-/);
...
test('Queue Mode preserves its normal playing-page URL for the MME handoff', () => {
    ...
    assert.match(command, new RegExp(`--detail-link '${watchUrl}'`));
    assert.match(command, /--save-name "crunchyroll-G9DUE3QXP-/);
    assert.match(command, /--media-folder "\$mediafab_media_file"/);
});
...
test('Crunchyroll Queue skips an MME-named episode recursively before downloading it', () => {
    ...
    writeFileSync(path.join(episodeFolder, 'S01E02 SPY x FAMILY - SECURE A WIFE.mkv'), 'existing');
...
test('Disney+ Queue skips an episode already organized by MME', () => {
...
test('shared normal builder preserves the public HLS fallback behavior', () => {
    ...
    assert.doesNotMatch(command, /(?:^|\s)-sv(?:\s|$)/);
    assert.match(command, /-M format=mkv --no-log -sa best -ss all/);
});
...
test('Crunchyroll and Disney+ keep automatic discovery inside the extension', async () => {
    assert.deepEqual(getRegisteredProviderIds(), ['crunchyroll', 'disneyplus']);
...
test('metadata handoff prioritizes a current Crunchyroll episode and preserves other-provider manual links', () => {
    ...
    assert.equal(
        resolveMMEDetailLink({ detailLink: 'https://manual.example/item' }, 'https://provider.example/play/1'),
